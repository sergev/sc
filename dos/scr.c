...
static initbold ()
{
	...
	if (MR)
		SO = MR;
	if (SO)
		NR = strcat (strcpy (colorp, skipdelay (ME)), skipdelay (SO));
	BS = MD;
	DS = MH;
}
...
# else
# define SETATTR(a,b,c,d,e,f)\
	if (a)\
		tputs (b);\
	else {\
		if ((curscr.flgs & (DIM|BOLD)) != (d))\
			tputs (c);\
		if (e(curscr.flgs & STANDOUT))\
			tputs (f);\
	}\
	break
...
register c;
{
	...
	if ((c & (DIM | BOLD | STANDOUT)) != (curscr.flgs & (DIM | BOLD | STANDOUT)))
		switch (c & (DIM | BOLD | STANDOUT)) {
		case 0:                 SETATTR (NR,NS,NS,0,,SE);
		case STANDOUT:          SETATTR (NR,NR,NS,0,!,SO);
		case DIM:               SETATTR (DR,DS,DS,DIM,,SE);
		case DIM|STANDOUT:      SETATTR (DR,DR,DS,DIM,!,SO);
		case BOLD:              SETATTR (BR,BS,BS,BOLD,,SE);
		case BOLD|STANDOUT:     SETATTR (BR,BR,BS,BOLD,!,SO);
		}
	curscr.flgs = c;
}
# endif
